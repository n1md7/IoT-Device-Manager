...
export function Schedules() {
	const { list, pins, loading, message, drafts, addDraft, removeDraft } = useSchedules();
	...
	if (pins.length === 0) {
		return (
			<section>
				<Message {...message} />
				<div class="card muted">Register a switch first — a schedule needs a switch to drive.</div>
			</section>
		);
	}
	...
	return (
		<section>
			...
			{drafts.map((key) => (
				<ScheduleCard key={`draft-${key}`} seed={draftSeed()} onChanged={() => removeDraft(key)} />
			))}
