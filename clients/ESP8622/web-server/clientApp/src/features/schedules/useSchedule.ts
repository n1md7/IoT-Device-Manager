...
import { confirm } from "@src/store/modal";
import {
  createSchedule,
  removeSchedule,
  updateSchedule,
} from "@src/store/schedules";
import {
  activeDays,
  toScheduleInput,
  toTimeInput,
...
export function useSchedule(seed: ScheduleSeed, onChanged?: () => void) {
    ...
    const ok = await track(() =>
      run(() =>
        (isNew
          ...
          : updateSchedule(seed.id!, payload)
        ).then(() => undefined),
      ),
    );
    if (ok) onChanged?.();
  ...
  const remove = useCallback(async () => {
    const confirmed = await confirm(
      `Schedule ${seed.id} will be permanently removed.`,
      {
