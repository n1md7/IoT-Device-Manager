...
import type { Schedule, ScheduleInput, ScheduleUpdate, TimePayload, WeekPayload } from '../api/types';
import { WEEK_KEYS } from '../utils/format';
...
const timeToString = (time: TimePayload): string => `${time.hh}:${time.mm}:${time.ss}`;
const weekToString = (week: WeekPayload): string => WEEK_KEYS.map((key) => week[key]).join(':');
...
export async function createSchedule(payload: ScheduleInput): Promise<Schedule | null> {
...
export async function updateSchedule(id: number, payload: ScheduleUpdate): Promise<void> {
	...
	schedules.value = schedules.value.map((s) => (s.id === id ? { ...s, ...fields } : s));
}
